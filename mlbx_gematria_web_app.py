...
st.caption("Includes Smart Bets, ROI, Prop Parlay Builder, and Script Mode")
...
if uploaded_file:
    ...
    if not all(col in df.columns for col in required):
        ...
        # Script Mode Scoring
        def ritual_rating(score):
            if score >= 90 and int(score) % 13 == 0:
                return "🔺 HIGH"
            elif score >= 80:
                return "🔹 MODERATE"
            else:
        ...
        df["Ritual_Alignment"] = df["Gematria_Alignment_Score"].apply(ritual_rating)

        st.success("✅ VIP Data + Script Mode Loaded")
        ...
        # Script Mode
        st.subheader("🎬 MLB Script Mode - Ritual Alignment Ratings")
        today = datetime.datetime.now()
        st.text(f"Date: {today.strftime('%A, %B %d, %Y')}")
        st.dataframe(df[["Matchup", "Gematria_Alignment_Score", "Narrative_Confidence_High", "Ritual_Alignment"]])

        # Download
        csv = df.to_csv(index=False).encode("utf-8")
        st.download_button("📥 Download All Results", csv, "MLBX_VIP_ScriptMode.csv", "text/csv")
